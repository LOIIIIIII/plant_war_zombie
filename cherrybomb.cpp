...
CherryBomb::CherryBomb()
{
    atk = atk_CherryBomb;
    hp = hp_NormalPlant;
    setMovie(":/plant/res\\Plant/CherryBomb.gif");
}

QRectF CherryBomb::boundingRect() const
{
    //正常状态和其他状态对范围内的碰撞进行检测消除僵尸
...
void CherryBomb::advance(int phase)
{
    ...
    {
        state = 1;
        setMovie(":/bgc/res\\Other/Boom.gif");
...
bool CherryBomb::collidesWithItem(const QGraphicsItem *other, Qt::ItemSelectionMode mode) const
{
    ...
    return true;//other->type() == Zombie::Type && QLineF(pos(), other->pos()).length() < 160;
}
