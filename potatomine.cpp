...
PotatoMine::PotatoMine()
{
    ...
    setMovie(":/plant/res\\Plant/PotatoMine1.gif");
}
...
void PotatoMine::advance(int phase)
{
    ...
    else if (state == 0 && ++counter >= time)
    {
        ...
        setMovie(":/plant/res\\Plant/PotatoMine.gif");
    }
    //遇到僵尸就爆炸
    else if (state == 1 && ++counter >= time)
    {
        ...
        {
            state = 2;
            setMovie(":/plant/res\\Plant/PotatoMineBomb.gif");
...
{
    Q_UNUSED(mode)
    return false;//other->type() == Zombie::Type && qFuzzyCompare(other->y(), y()) && qAbs(other->x() - x()) < 50;
}
